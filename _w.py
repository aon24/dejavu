...
class wsgiRH(BaseHTTPRequestHandler):
    ...
    def log_message(self, frmat, *args):
        pass
        # snd('addr %r, %s' % (self.client_address, frmat % args), cat='http-server')
    ...
    def handle_one_request(self):
        try:
            ...
            self.log_error('svServer.py.handle_one_request:\n%r', ex)
            self.close_connection = True
...
if __name__ == '__main__':
    import dejavu.wsgi as app
    from tools.DC import config

    wsgiApplication = app.application
    port = int(config.httpPort or 0) or 80
    server = SovaHttpServer(('', port), wsgiRH)
    threading.Thread(target = server.serve_forever).start()
    snd(f'''
=== === === HTTP WSGI server started on {config.ws_server}:{port}
=== === === {versionString}
