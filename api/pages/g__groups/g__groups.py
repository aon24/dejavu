...
class g__groups(Page):
    ...
    def __init__(self, dcUK):
        ...
        self.dbAlias = 'groups'
        self.cats = {
            'Группы': DCC(condition='True', sort='groupName', reverse=False,
                fixedSubCats=[
                    DCC(subCat='Общие', condition='not d.tracks'),
                    DCC(subCat='Служебные', condition='d.system'),
                ]
            ),
            'По\xa0номеру': DCC(condition='True', sort='docNo', reverse=True,
                fixedSubCats=[
                    DCC(subCat='Общие', condition='not d.tracks'),
                    DCC(subCat='Служебные', condition='d.system'),
                ]
            ),
            'По\xa0курсам': DCC(condition='d.tracks', sort='groupName', subCat='d.tracks'),
        }
    ...
    def page(self, dcUK):
        ...
        dcVP = DCC(
            ...
            viewStyle=dict(width=200),  # стиль вида
            form='g_group',
            dbAlias=dcUK.dbAlias or 'groups',
        )
