...
import api.reviews.reviews as views
...
from websockets.headers import peek_ahead
...
class Page(object):
    ...
    def getUrl(self, dcUK):
        ...
        '''
        key = '-'.join([dcUK.dbAlias or self.dbAlias, dcUK.mode, dcUK.multiPage, dcUK.userAgent])
...
#                     url += eval(td[3])
    ...
    def getViewObject(self, className, dcVP):
        key = f"{className.upper()}-{dcVP['dbAlias']}"
        view = well('viewObjects', key)
        
        if not view:
            v = getattr(views, className, None)
            if v:
