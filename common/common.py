...
def notFound(s, un=''):
    ...
    return HttpResponse(f'''<h3>{s}</h3> not found ({un or 'userName'})''', None, 404)

def jsonNotFound(s='', un=''):
    snd(s or '-?-', cat='json-object not found')
    return f'''"{s or 'block not found'} ({un or 'userName'})"''', 'application/json', 404
        ...
        for s in ls:
            if 'jsv?' not in s:
                ...
                try:
                    ...
                    err('js or css not found', cat=fn)
    return ols
